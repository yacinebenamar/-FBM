...
export default function TaskSignatureModal({ task, isOpen, onClose, onConfirm }: TaskSignatureModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-[9999] flex items-center justify-center p-4" dir="rtl">
      <div className="bg-[#050E46] border border-slate-800 w-full max-w-lg rounded-3xl p-6 shadow-2xl space-y-5 animate-scaleIn text-right">
        ...
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-black text-white">تأكيد الإنجاز وتوقيع المهمة</h3>
              <p className="text-[10px] text-slate-400">التوقيع الرقمي وإثبات الموقع الميداني للأندرويد</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        {/* Task Title Card */}
        <div className="bg-[#000839] p-3 rounded-2xl border border-slate-800">
        ...
        <div className="space-y-2">
          ...
          <div className="relative border-2 border-dashed border-slate-800 rounded-2xl overflow-hidden bg-[#000839]">
            <canvas
              ref={canvasRef}
        ...
        <div className="flex gap-2.5 pt-2">
          ...
          <button
            onClick={onClose}
            className="bg-slate-900 border border-slate-800 text-slate-300 hover:text-white font-bold px-5 py-3 rounded-xl text-xs cursor-pointer transition-all duration-200"
          >
            إلغاء
          </button>
        </div>
