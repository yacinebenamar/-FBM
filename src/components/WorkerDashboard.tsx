...
export default function WorkerDashboard({
  ...
  useEffect(() => {
    ...
    if (isAllowed('canViewAllExpenses')) allowedTabs.push('expenses');
    if (isAllowed('canManageTasks')) allowedTabs.push('tasks');
    if (isAllowed('canManageTransfers')) allowedTabs.push('transfers');
    if (isAllowed('canManageCamion')) allowedTabs.push('camion');
    if (isAllowed('canManageAttendance')) allowedTabs.push('attendance');
    if (isAllowed('canManageOrders')) allowedTabs.push('orders');
    allowedTabs.push('chat');
  ...
  const myTasks = tasks.filter(t => t.assignedToUids && t.assignedToUids.includes(currentUser.uid));
  const myNotifications = notifications.filter(n => n.targetType === 'all' || (n.targetUids && n.targetUids.includes(currentUser.uid)));
  const unreadNotifsCount = myNotifications.filter(n => !n.readByUids || !n.readByUids.includes(currentUser.uid)).length;
  ...
  const handleClockIn = async () => {
    ...
    setTimeout(() => {
      ...
      const pTime = new Date().toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' });
      onAddChatMessage({
  ...
  return (
    <div className="min-h-screen bg-[#000839] text-white flex flex-col pb-16 select-none">
      ...
      <div className="flex-1 max-w-4xl w-full mx-auto p-4 pb-28 space-y-4 overflow-y-auto" dir="rtl">
        ...
        <div className="bg-[#050E46] border border-slate-800 p-3.5 rounded-2xl flex items-center justify-between text-xs text-right gap-3">
          <div className="flex items-center gap-2">
            <Sparkles className="text-[#76BC21] w-4 h-4 shrink-0 animate-pulse" />
            <span className="text-slate-300 font-medium">المزامنة الميدانية والتوجيه اليومي: <strong className="text-emerald-400">نشط</strong></span>
          </div>
          <span className="text-[10px] bg-[#000839] px-2.5 py-1 rounded-lg border border-slate-800 text-slate-500 font-mono">FBM-ONLINE</span>
        </div>

        <div className="grid gap-3 md:grid-cols-[1.2fr_0.8fr]">
          <div className="rounded-2xl border border-slate-800/80 bg-gradient-to-l from-[#050E46] to-[#0a186b] p-4 text-right">
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#76BC21]">ملخص يومي</p>
            <h3 className="mt-2 text-base font-black text-white">تابع مهامك ومصاريفك دون مغادرة الشاشة</h3>
            <p className="mt-2 text-sm text-slate-400">الواجهة مصممة لتكون أسرع على الهاتف مع زر إضافة واضح ومباشر.</p>
          </div>
          <div className="rounded-2xl border border-slate-800/80 bg-[#050E46] p-4 text-right">
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">إجراءات سريعة</p>
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                onClick={() => setShowExpenseModal(true)}
                className="rounded-xl border border-[#76BC21]/20 bg-[#76BC21]/10 px-3 py-2 text-[11px] font-black text-[#76BC21] transition-all hover:bg-[#76BC21]/20"
              >
                + مصروف جديد
              </button>
              <button
                onClick={() => setActiveTab('tasks')}
                className="rounded-xl border border-slate-800 bg-[#000839] px-3 py-2 text-[11px] font-black text-slate-200 transition-all hover:border-slate-700"
              >
                المهام
              </button>
              <button
        ...
        {activeTab === 'expenses' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-black text-white">سجل المصاريف الفورية</h2>
              <button
                onClick={() => setShowExpenseModal(true)}
                className="bg-[#76BC21] hover:bg-[#62a118] text-[#000839] px-4 py-2 rounded-xl text-xs font-black transition-all flex items-center gap-2 shadow-lg shadow-[#76BC21]/20"
              >
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-[#050E46] border border-slate-800/80 p-4 rounded-2xl text-right">
                ...
                <h3 className="text-sm font-black text-[#76BC21] font-mono">{approvedWorkerExpensesSum.toLocaleString('en-US')} د.ج</h3>
              </div>
              <div className="bg-[#050E46] border border-slate-800/80 p-4 rounded-2xl text-right">
                <span className="text-[10px] text-slate-500 font-bold block mb-1">قيد المراجعة</span>
                <h3 className="text-sm font-black text-amber-500 font-mono">{pendingWorkerExpensesSum.toLocaleString('en-US')} د.ج</h3>
              </div>
            </div>
                ...
                {myExpenses.map((expense) => (
                  ...
                  >
                    ...
                    <div className="text-left shrink-0 space-y-2 flex flex-col items-end">
                      <span className="block text-sm font-black text-white font-mono">{expense.amount.toLocaleString('en-US')} د.ج</span>
                        ...
                        <div className="flex flex-wrap gap-1.5 items-center justify-start">
                          ...
                          {task.dueDate && (
                            <span className={`text-[9px] px-2 py-0.5 rounded-full font-bold flex items-center gap-1 ${
                              ...
                              <span>الموعد: {new Date(task.dueDate).toLocaleString('en-GB', {
                                month: 'short',
                                day: 'numeric',
                                hour: '2-digit',
                                minute: '2-digit'
                      ...
                      <div className="bg-[#000839]/40 p-2.5 rounded-xl border border-slate-800 text-[10px] text-slate-400 space-y-1">
                        {task.startedAt && (
                          <div className="flex justify-between items-center">
                            ...
                            <span className="font-mono text-slate-300">{new Date(task.startedAt).toLocaleString('en-GB')}</span>
                          </div>
                        )}
                        {task.completedAt && (
                  ...
                  return (
                    <div className="bg-gradient-to-br from-[#050E46] to-emerald-950/20 p-5 border border-emerald-500/20 rounded-xl space-y-4 text-center">
                      ...
                      <div className="space-y-1">
                        ...
                        <h4 className="text-xs font-black text-white pt-2">بداية العمل: {new Date(activeRecord.clockInTime).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}</h4>
                    ...
                    {attendance.filter(r => r.workerUid === currentUser.uid).map(rec => (
                      <div key={rec.id} className="bg-[#000839] border border-slate-800/80 p-3 rounded-xl flex items-center justify-between gap-3 text-right">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <h5 className="text-[11px] font-bold text-white">{new Date(rec.clockInTime).toLocaleDateString('en-GB', { month: 'short', day: 'numeric' })}</h5>
                            <span className="text-[9px] text-slate-400 font-mono">
                              {new Date(rec.clockInTime).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}
                              {rec.clockOutTime ? ` - ${new Date(rec.clockOutTime).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}` : ' (مستمر)'}
                            </span>
                          </div>
                        </div>
            ...
            <div className="bg-[#050E46] border border-slate-800 rounded-2xl p-5 space-y-4 text-right flex flex-col h-[500px]">
              ...
              <div className="flex-1 overflow-y-auto space-y-3 p-1 select-text">
                {chatMessages.filter(msg => msg.senderUid !== 'system').length === 0 ? (
                  ...
                  chatMessages.filter(msg => msg.senderUid !== 'system').map((msg) => {
      ...
      <div className="fixed bottom-4 left-4 right-4 bg-[#050E46]/95 backdrop-blur-lg border border-slate-800/80 p-2.5 flex overflow-x-auto flex-nowrap scrollbar-none gap-2 items-center justify-around z-45 max-w-4xl mx-auto rounded-2xl shadow-2xl" style={{ scrollbarWidth: 'none' }}>
        {isAllowed('canViewAllExpenses') && (
          <button
          ...
          >
            ...
            <span className="text-[9px] font-bold">المصاريف</span>
          </button>
        )}

        {isAllowed('canManageTasks') && (
          <button
            onClick={() => setActiveTab('tasks')}
          ...
          >
            ...
            <span className="text-[9px] font-bold">المهام</span>
          </button>
        )}

        {isAllowed('canManageTransfers') && (
          <button
          ...
          >
            ...
            <span className="text-[9px] font-bold">حركة السلع</span>
          </button>
        )}

        {isAllowed('canManageOrders') && (
          <button
          ...
          >
            ...
            <span className="text-[9px] font-bold">الطلبيات</span>
            {clientOrders.filter(o => o.status === 'pending').length > 0 && (
              <span className="absolute top-0.5 right-2 bg-emerald-500 text-white font-mono text-[8px] px-1 rounded-full scale-90 font-bold">
        ...
        {isAllowed('canManageCamion') && (
          ...
          >
            ...
            <span className="text-[9px] font-bold">سائق الشاحنة</span>
          </button>
        )}

        {isAllowed('canManageAttendance') && (
          <button
          ...
          >
            ...
            <span className="text-[9px] font-bold">تسجيل الحضور</span>
          </button>
        )}
